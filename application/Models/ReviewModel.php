...
class ReviewModel{
    ...
    public function add_review($firstname, $lastname, $review, $rating=5){
		$statement = $this->db->query("INSERT INTO review (firstname, lastname, review, rating) VALUES(?,?,?,?)", $params=[$firstname, $lastname, $review, $rating]);
		if($statement){ return true;}
		else{return false;}
	}

    public function getAllReviews(){
        $statement = $this->db->query("SELECT * FROM $this->tbl", $params=[]);
        if($statement){
            return $statement;
        }else{
            return false;
